function sents = genSentencePool(nSent)
%%
orig_sents=...
...
%% Calculate the number of syllables in each sentence
dictFN='./beep';
dict=textread(dictFN,'%s','delimiter','\n');

orig_nSyls=nan(size(orig_sents));
for i1=1:length(orig_sents)
    t_sent=orig_sents{i1};
    idxsp=strfind(t_sent,' ');
    nWords=length(idxsp)+1;
    
    t_nSyls=zeros(1,nWords);

    for i2=0:length(idxsp)
        if i2==0
            t_word=t_sent(1:idxsp(1)-1);
        elseif i2<length(idxsp)
            t_word=t_sent(idxsp(i2)+1:idxsp(i2+1)-1);
        else
            t_word=t_sent(idxsp(i2)+1:end);
        end
        
        t_word=lower(t_word);
        bWordFound=0;
        for i3=1:length(dict);
            dict_line=dict{i3};
            idxsp1=strfind(dict_line,sprintf('\t'));
            if isempty(idxsp1)
                idxsp1=strfind(dict_line,' ');
            end  
            idxsp1=idxsp1(1);
            dict_word=lower(dict_line(1:idxsp1-1));
            if isequal(dict_word,t_word)
                bWordFound=1;
                break;
            end
        end
        ...
        for i3=idxsp1:length(dict_line)
            if isletter(dict_line(i3)) && isequal(lower(dict_line(i3)),dict_line(i3))
                break;
            end
        end
        dict_line=dict_line(i3:end);
        idxsp2=strfind(dict_line,' ');       
        
        for i3=0:length(idxsp2)
            if isempty(idxsp2)
                t_phone=dict_line;
            else
                if i3==0
                    t_phone=dict_line(1:idxsp2(1)-1);
                elseif i3<length(idxsp2)
                    t_phone=dict_line(idxsp2(i3)+1:idxsp2(i3+1)-1);
                else
                    t_phone=dict_line(idxsp2(i3)+1:end);
                end
            end
            
                       
            
            if is_phone_vowel(t_phone)
                t_nSyls(i2+1)=t_nSyls(i2+1)+1;
            end
        end
    end
    orig_nSyls(i1)=sum(t_nSyls);
    
    fprintf('Sentence %d of %d done.\n',i1,length(orig_sents));
    save('sentencePool.mat','orig_sents','orig_nSyls');
end
